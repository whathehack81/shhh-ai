...
@click.group()
@click.version_option("1.0.1", prog_name="gitleaks-ai")
...
def scan(ctx, target, ai_review, min_entropy, output, report, no_fp, max_file_size, batch_size, rate_limit_delay):
    ...
    try:
        ...
        # Warn if --no-fp used without --ai-review
        ...
        if report:
            try:
                ...
                with open(report, "w") as f:
                    ...
                    f.write(f"**Scan Duration:** {(datetime.now() - scan_start).total_seconds():.2f}s\n\n")
                    f.write("---\n\n")
                    f.write(remediation)
